...
bool EPosixClientSocket::handleSocketError()
{
	// no error
	if( errno == 0){
		//std::cout<<"0==EPosixClientSocket::handleSocketError()"<<std::endl;
		return true;
	}

	// Socket is already connected
	if( errno == EISCONN) {
		std::cout<<"1==EPosixClientSocket::handleSocketError()"<<std::endl;
		return true;
	}

	if( errno == EWOULDBLOCK){
		std::cout<<"2==EPosixClientSocket::handleSocketError()"<<std::endl;
		return false;
	}

	if( errno == ECONNREFUSED) {
		std::cout<<"3==EPosixClientSocket::handleSocketError()"<<std::endl;
		getWrapper()->error( NO_VALID_ID, CONNECT_FAIL.code(), CONNECT_FAIL.msg());
	}
	else {
		std::cout<<"4==EPosixClientSocket::handleSocketError()"<<std::endl;
	...
	errno = 0;
	eDisconnect();
	std::cout<<"5==EPosixClientSocket::handleSocketError()"<<std::endl;
	return false;
}
