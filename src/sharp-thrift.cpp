...
public:
	...
	void reqHistoricalData(std::map<int64_t, std::string> & m, const api::HistoryRequest & request){
		protect( [this, &m, &request](){
			Contract con;
			con.symbol = request.symbol;
			con.secType = request.secType;
			...
			auto id = std::get<0>(trader.hist_data_tuple);
			m[id] = std::get<2>(trader.hist_data_tuple);
		} );
	}

	void cancelHistoricalData (const int64_t tickerId){
		protect( [this, tickerId](){
			trader.cancelHistoricalData(tickerId);
		} );
	}
